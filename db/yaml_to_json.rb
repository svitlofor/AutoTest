...
class YamlToJson
  ...
  def create_folders(folder_ = nil)
    iter_folder = folder_ || File.dirname(@json_file_name)
    folders = []
  ...
  def self.copy_images(folder_from, folder_to)
    puts "Copying images from #{folder_from} to folder #{folder_to}"
    Dir.entries(folder_from).each do |name|
      ...
      if File.directory?(full_path_from)
        copy_images(full_path_from, full_path_to)
        next
      end
      self.new.create_folders(File.dirname(full_path_to))
      command = "cp #{full_path_from} #{full_path_to}"
...
YamlToJson.convert_folder(File.absolute_path('./../spec/test1/tests/'), File.absolute_path('db/tests'))
